  ...
  useEffect(() => {
    const setupContracts = async () => {
      ...
      try {
        ...
        const evAddress = EventsABI.networks[networkId]?.address;

        if (!tfAddress || !evAddress) {
          console.error('Missing contract address');
          return;
        }

        const tfContract = new ethers.Contract(tfAddress, TicketFactoryABI.abi, signer);
        const evContract = new ethers.Contract(evAddress, EventsABI.abi, signer);
          ...
          {filteredEvents.map((event) => (
            ...
            >
              ...
              <div className="h-44 overflow-hidden">
                <img
                  src={event.imageUrl || "https://via.placeholder.com/800x450?text=No+Image"}
                  alt={event.title}
