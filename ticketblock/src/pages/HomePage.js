...
const [selectedPlace, setSelectedPlace] = useState("");
  ...
  {allEvents.map((event, index) => {
    ...
    return (
      <div key={`${event.eventId.toNumber()}_${index}`} className="p-3">
        <div className="bg-neutral-900 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition relative">
          ...
          <div className="h-56 overflow-hidden">
            <img
              src="https://images.unsplash.com/photo-1503516658610-b3a9e4d1b691?q=80&w=1674&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
              alt={event.title}
          ...
          <div className="p-4">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-400 mb-4">
              ...
              <span>Starting: ${event.price || "200"}</span>
            </div>

            {/* Botón */}
            <Link
              to={isExample ? "#" : `/event/${event.eventId.toNumber()}`}
              className={`inline-block text-sm font-medium px-4 py-2 rounded-full transition duration-300 ${
                isExample
      ...
      {/* PROMOCIONAL TEXTO EXTRA */}
   <section className="relative py-16 text-center text-white">
  {/* Imagen de fondo */}
  <div
    className="absolute inset-0 bg-cover bg-center opacity-30"
    style={{
      backgroundImage: `url('https://images.unsplash.com/photo-1544509807-aabbcf9c5cba?q=80&w=2066&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')`,
    }}
  ></div>
  ...
  <div className="relative z-10 px-4">
    ...
    <p className="text-gray-200 text-lg">
      Explora, filtra y encuentra el evento perfecto para ti
    </p>
  </div>
</section>
<h2 className="text-3xl md:text-4xl font-bold mt-10 text-center text-white">
          Todos los eventos
        </h2>
  ...
  <h2 className="text-2xl font-semibold text-white">Eventos por auditorio</h2>
  ...
  <div className="overflow-x-auto mb-8 pb-2">
    <div className="flex gap-2 w-max">
      ...
      {[...new Set(allEvents.map((e) => e.place || "Desconocido"))].map((place) => (
        <button
          key={place}
...
>
  <img
    src="https://images.unsplash.com/photo-1543748693-d5cb2d062a06?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
    alt={event.title}
